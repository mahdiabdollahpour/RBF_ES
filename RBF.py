...
def classification_evaluator(loss, x_data, y_data, individual):
    nums = len(individual)
    dim = len(x_data[0])
...
def regression_evaluator(loss, x_data, y_data, individual):
    nums = len(individual)
    dim = len(x_data[0])
    number_of_classes = max(y_data) + 1
    circle_nums = nums / (dim + 1)
    ...
    for i in range(L):
        for j in range(m):
            ...
            v = individual[(dim + 1) * (j - 1):(dim + 1) * (j - 1) + dim]
            G[i][j] = g_func(v, sigma=individual[(dim + 1) * (j - 1) + dim + 1], x=x_data[i])
    W = np.matmul(np.matmul(np.linalg.inv(np.matmul(np.transpose(G), G) + landa * np.eye(m)), np.transpose(G)), y_data)
    y_hat = np.matmul(G, W)
    l = loss(y_data, y_hat)
    print("regression loss?", l)
    return  l,


def get_y_classification(individual, x_data, y_data):
    nums = len(individual)
    dim = len(x_data[0])
    number_of_classes = max(y_data) + 1
    circle_nums = nums / (dim + 1)
...
def get_y_regression(individual, x_data, y_data):
    nums = len(individual)
    dim = len(x_data[0])
    number_of_classes = max(y_data) + 1
    circle_nums = nums / (dim + 1)
