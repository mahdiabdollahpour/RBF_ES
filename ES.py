...
def find_circle_coordinates(x_train, y_train, number_of_circles, evaluator, loss, NGEN=10, MIN_VALUE=-1.5,
                            MAX_VALUE=1.5, MIN_STRATEGY=0.5, MAX_STRATEGY=3, MU=10, LAMBDA=20):
    dim = len(x_train[0])
    chromosome_size = (dim + 1) * number_of_circles

    es = ES(evaluator, loss, x_train, y_train, chromosome_size, MIN_VALUE, MAX_VALUE, MIN_STRATEGY,
            MAX_STRATEGY)

    pop, logbook, hof = es.solve_problem(NGEN=NGEN, MU=MU, LAMBDA=LAMBDA)
    # print(pop)
    # print(np.shape(pop))
    # print(len(pop[0]))
...
class ES():
    def __init__(self, evaluate, loss, x_data, y_data, IND_SIZE, MIN_VALUE=-1.5, MAX_VALUE=1.5, MIN_STRATEGY=0.5,
        ...
        self.toolbox.register("evaluate", self.evaluate, loss, x_data, y_data, x_data, y_data)
